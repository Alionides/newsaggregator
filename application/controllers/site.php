...
public function ghjk(){
    ...
    foreach ($results as $key => $value)
    {
       ...
       // $cleanslug = convert_accented_characters($value['site_title']);
        ...
        $lang = preg_match('/&#10[78]\d/', mb_encode_numericentity($value['site_title'], array(0x0, 0x2FFFF, 0, 0xFFFF), 'UTF-8'));
...
public function olkeazjsonapi() {


$arr = array(
  array('title' => 'bakida guclu kulek qopacag', 'url' => 'http://az.surf.az/site/tapdim/73598', 'img_url' => 'http://surf.az/images/surfaz.jpg'),
    array('title' => 'bakida guclu kulek qopacag', 'url' => 'http://az.surf.az/site/tapdim/73598', 'img_url' => 'http://surf.az/images/surfaz.jpg'));    
...
echo json_encode($arr);
    ...
    public function surf_news(){          
        ...
        $data["reklams"] = $this->model_get->getReklams();          
        $config = array();
        ...
        // $config['next_tag_open'] = '<li class="waves-effect"><i class="material-icons">chevron_right</i>';
        // $config['next_tag_close'] = '</li>';
        // $config['prev_tag_open'] = '<li class="waves-effect"><i class="material-icons">chevron_left</i>';
        ...
        $config['cur_tag_open'] = '<li class="active light-blue darken-2"><a href="#!">';
        $config['cur_tag_close'] = '</a></li>';
        $config['next_link'] = '<li class="waves-effect"><i class="material-icons">chevron_right</i>';
        $config['prev_link'] = '<li class="waves-effect"><i class="material-icons">chevron_left</i>';
        $this->pagination->initialize($config);
        $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $data["results2"] =  $this->model_get->fetch_countries($config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();

        $this->load->view("mainheader",$data);
        $this->load->view("surf_news");
...
public function surf_cat($cat){
       ...
       public function newsearch(){
             ...
             $match = ($this->input->post("search"))? $this->input->post("search") : "NIL";

             $match = ($this->uri->segment(3)) ? $this->uri->segment(3) : $match;

             $last_id=false;
             
              $data["results"] = $this->model_get->getSites($last_id);
              $config = array();
              $config["base_url"] = base_url() . "site/newsearch/$match";
              $config["total_rows"] = $this->model_get->num_search_messages($match);  
              ...
              $page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
              $data["results2"] =  $this->model_get->get_newsearch($config["per_page"], $page,$match);
...
public function tapdim($last_id = FALSE)
       {
        ...
        $data["similarnews"] = $this->model_get->getsimilarnews($key['site_cat']);
        ...
        public function fbinsertru(){
          $this->load->library('facebook');

          $this->load->model("model_get");
          
          $results = $this->model_get->getfblastnewsru();
          
          foreach ($results as $key){}

          $siteid=$key->site_id+1;

          $md5val['md5url'] = $key->md5url;

          $checkmd5 = $this->model_get->checkfbshared($md5val);

          if($checkmd5 == 1){ 

          $page_access_token = 'EAAFedWpXIV0BAAIWRl9j8ZB2qIXeXeeoTk48lHja4i2TLImkpxd4MnGH9P4PfiT0C9nuIjQcCb6Y68LGKYfJ3lcvRN1swdR4Pi0jimbcYemY6X9epHWRppJc3MLl8OoueE1anIWWlLQq9VynOFwntUMSRxe7m9h7ZB2ItkPQZDZD';
          $page_id = '761859787200570';
          $data['picture'] = $key->site_img_url;
          $data['link'] = "http://surf.az/site/tapdim/".$siteid."/".$key->slug;
          $data['message'] = strip_tags($key->site_title);
          $data['caption'] = "SURF.AZ";
          $data['description'] = strip_tags($key->site_desc);
          ...
          $newdata['md5url'] = $key->md5url;
          $this->model_get->insertfbsharedmd5($newdata);


        }else{
          echo "site already shared on fb";
        }



        }
public function fbinsertSONXEBER(){
          $this->load->library('facebook');

          $this->load->model("model_get");
          //$data["results"] = $this->model_get->getfblastnews();
          $results = $this->model_get->getfblastnews();
          //foreach ($data["results"] as $key) {} 
          foreach ($results as $key){}
          ...
          $page_access_token='EAAZAMQlG7d10BALuoHLCM3y1O9CzMtJJST8MfZCTZBMaKRQy9HRSm66H6yZCQw5w8SKoLIi9xZCtx4FxZCkWunDkEB7eayavpdgNuk2CCRz0qgFsp3pNu98JaiHIZA0hWgOPXeaFVnvmMllEHH90ddOiBgrp7KBssQwuq2qePxV7wZDZD';
          $page_id = '110201869169740';
          $data['picture'] = $key->site_img_url;
          $data['link'] = "http://az.surf.az/site/tapdim/".$siteid."/".$key->slug;
          $data['message'] = strip_tags($key->site_title);
          $data['caption'] = "AZ.SURF.AZ";
          $data['description'] = strip_tags($key->site_desc);
          $data['access_token'] = $page_access_token;
          $post_url = 'https://graph.facebook.com/'.$page_id.'/feed';
...
public function lentazshowcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://news.lent.az/category/44');

foreach($html->find('.category-news-list a') as $e) { 
  $array[] = $e->href;
}

 $url = 'http://news.lent.az'.$array[0];
...
$source='<br>'.'mənbə: '.'<a href="http://news.lent.az/" target="_blank">'.'Lent.az'.'</a>';
$allcontent = $content.$source;
$croncat = 10;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
$data['lang']=0;
$data['md5url']=md5($url);
$data['sitecat']=$croncat;
$data['basliq']= str_replace('"', '', $maintitle);
$cleanslug = convert_accented_characters($maintitle);
...
public function bizpluscron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://bizplus.az/');

foreach($html->find('.td-ss-main-content a') as $e) { 
  $array[] = $e->href;
}
...
foreach($html->find('.td-post-content') as $e) { 
...
$source='<br>'.'mənbə: '.'<a href="http://bizplus.az/" target="_blank">'.'Bizplus.az'.'</a>';
$allcontent = $content.$source;
$croncat = 2;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function qafqazinfosocialcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://qafqazinfo.az/news/category/sosial-3');

foreach($html->find('.panel-body a') as $e) { 
  $array[] = $e->href;
}
...
foreach($html->find('.news_text') as $e) { 
...
$source='<br>'.'mənbə: '.'<a href="http://qafqazinfo.az" target="_blank">'.'Qafqazinfo.az'.'</a>';
$allcontent = $content.$source;
$croncat = 4;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function qafqazinfoshowcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://qafqazinfo.az/news/category/sou-20');
...
public function azxebermaqazincron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://azxeber.com/az/maqazin/');

foreach($html->find('.col-xs-6 a') as $e) { 
  $array[] = $e->href;
}
...
$source='<br>'.'mənbə: '.'<a href="http://azxeber.com" target="_blank">'.'Azxeber'.'</a>';
$allcontent = $content.$source;
$croncat = 10;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function admerucron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('https://www.adme.ru/');

foreach($html->find('.content a') as $e) { 
  $array[] = $e->href;
}

 $url = 'https://www.adme.ru'.$array[0];

 $html = file_get_html($url);

 foreach($html->find('.article h1') as $e) { 
}

$maintitle = $e->innertext;
...
$content = strip_tags($e->children(3)->innertext,"<p>"); 

foreach($html->find('.article img') as $e) {   
  $allimgarray[] = "<img src='".$e->src."'/><br><br>";
}

$allimages = implode('', $allimgarray).'<br><br>';
$maintext = $content.$allimages; 
...
$source='<br>'.'источник: '.'<a href="http://adme.ru" target="_blank">'.'Adme.ru'.'</a>';
$allcontent = $maintext.$source;
$croncat = 15;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
$data['lang']=1;
...
public function gappazrucron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://gapp.az/raznoe');

foreach($html->find('.cat_list a') as $e) { 
  $array[] = $e->href;
}

 $url = 'http://gapp.az'.$array[0];

 $html = file_get_html($url);

 foreach($html->find('.n_title_wrap h1') as $e) { 
}

$maintitle = $e->innertext;


foreach($html->find('.ncl_left img') as $e) { 
  $allimgarray[] = "<img src='".$e->src."'/><br><br>";
}
...
$source='<br>'.'источник: '.'<a href="http://gapp.az" target="_blank">'.'Gapp.az'.'</a>';
$allcontent = $maintext.$source;
$croncat = 15;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function trendrucron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://trend.az/latest/');

foreach($html->find('.section-content a') as $e) { 
  $array[] = $e->href;
}
 ...
 foreach($html->find('.title-news') as $e) { 
}

$maintitle = $e->innertext;

foreach($html->find('#article_content') as $e) { 
}
 
$maintext = $e->innertext; 
...
$source='<br>'.'mənbə: '.'<a href="http://trend.az" target="_blank">'.'Trend.az'.'</a>';
$allcontent = $maintext.$source;
$croncat = 1;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function trendazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://az.trend.az/latest/');
...
public function technoteazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://technote.az/');

foreach($html->find('.article-list-vertical a') as $e) { 
  $array[] = $e->href;
}
 ...
 foreach($html->find('.ddd h1') as $e) { 
}

$maintitle = strip_tags($e->innertext,"<h1>");

foreach($html->find('.post article') as $e) { 
}
...
$source='<br>'.'mənbə: '.'<a href="http://technote.az" target="_blank">'.'Technote.az'.'</a>';
$allcontent = $maintext.$source;
$croncat = 6;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function feedazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://feed.az/');

foreach($html->find('#content_box a') as $e) { 
  $array[] = $e->href;
}
 ...
 foreach($html->find('.entry-title') as $e) { 
}

$maintitle = strip_tags($e->innertext,"<h1>");

foreach($html->find('.entry-content') as $e) { 
}
...
$source='<br>'.'mənbə: '.'<a href="http://feed.az" target="_blank">'.'Feed.az'.'</a>';
$allcontent = $maintext.$source;
$croncat = 15;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function oxuazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://oxu.az/society');

foreach($html->find('.news-list a') as $e) { 
 $array[] = $e->href;
}

 $url = 'http://oxu.az'.$array[0];

 $html = file_get_html($url);

 foreach($html->find('.news-inner h1') as $e) { 

}

$maintitle = strip_tags($e->innertext,"<h1>");

foreach($html->find('.news-inner') as $e) { 
}
 
$innertext = $e->innertext;
$maintext = strstr($innertext, 'oxu.az' , true);
// $maintext = $e->innertext;

foreach($html->find('.news-img') as $e) { 
    $e->src;
}

$mainimage = $e->src;

foreach($html->find('.news-inner img') as $e) { 
  $allimgarray[] = "<img src='".$e->src."'/><br><br>";
}
if(isset($allimgarray)){
 $allimages = implode('', $allimgarray).'<br><br>';
}else{
 $allimages = "<img src='".$mainimage."'/>";
}


$source='<br><br>'.'источник: '.'<a href="http://oxu.az" target="_blank">'.'Oxu.az'.'</a>';
$allcontent = $maintext.$allimages;
$croncat = 4;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
$data['lang']=0;
...
public function haqqinazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://haqqin.az/news');
...
$maintitle = strip_tags($e->innertext,"");
...
foreach($html->find('.article-content') as $e) { 
    
}
 
$maintext = $innertext = strip_tags($e->innertext,"<p>").'<br><br>';
...
$source='<br>'.'источник: '.'<a href="http://haqqin.az" target="_blank">'.'Haqqin.az'.'</a>';
$allcontent = $maintext.$source;
$croncat = 1;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function autoazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://www.auto.az/news/');

foreach($html->find('.center_col a') as $e) { 
 $array[] = $e->href;
}

 $url = 'http://auto.az'.$array[0];

 $html = file_get_html($url);

 foreach($html->find('.art_content') as $e) { 

}



$maintitle = strip_tags($e->children(2)->innertext,"<h3>");



foreach($html->find('.art_content img') as $e) { 
...
$innertext = strip_tags($e->innertext,"<p>").'<br><br>';
$maintext = strstr($innertext, 'AUTO.AZ' , true).'<br><br>';





$source='<br>'.'источник: '.'<a href="http://auto.az" target="_blank">'.'Auto.az'.'</a>';
$allcontent = $maintext.$allimages.$source;
$croncat = 12;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
$data['lang']=0;
...
public function fishkinetcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://old.fishki.net/');

foreach($html->find('.fishki-post-list a') as $e) { 
 $array[] = $e->href;
}

 $url = 'http://fishki.net'.$array[0];
...
foreach($html->find('.picture-holder img') as $e) { 
...
$source='<br>'.'источник: '.'<a href="http://fishki.net" target="_blank">'.'Fishki.net'.'</a>';
$allcontent = $maintext.$allimages.$source;
$croncat = 15;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function puretrucron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://www.pure-t.ru/');

foreach($html->find('.slide a') as $e) { 
 $array[] = $e->href;
}
...
foreach($html->find('.entry-content img') as $e) { 
...
$innertext = strip_tags($e->children(2)->innertext,"");

$maintext = strstr($innertext, 'if' , true).'<br><br>';



$source='<br>'.'источник: '.'<a href="http://pure-t.ru" target="_blank">'.'Pure'.'</a>';
$allcontent = $maintext.$allimages.$source;
$croncat = 15;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function kenqurucomcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://kenquru.com/news');

foreach($html->find('.items_lists a') as $e) { 
  $array[] = $e->href;
}
$url = 'http://kenquru.com'.$array[0];

$html = file_get_html($url);

foreach($html->find('.post-title') as $e) { 
}
$maintitle = $e->innertext;

foreach($html->find('.media img') as $e) { 
}

$mainimage = $e->src;

foreach($html->find('.post-body') as $e) { 
}
 
$maintext = $e->innertext;


$source='<br>'.'источник: '.'<a href="http://kenquru.com" target="_blank">'.'Kenquru.com'.'</a>';
$allcontent = $maintext.$source;
$croncat = 15;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function pahooazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://pahoo.az/news');

foreach($html->find('.items_lists a') as $e) { 
  $array[] = $e->href;
}
$url = 'http://pahoo.az'.$array[0];
...
$source='<br>'.'источник: '.'<a href="http://pahoo.az" target="_blank">'.'Pahoo.az'.'</a>';
$allcontent = $maintext.$source;
$croncat = 15;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
public function axsamazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://axsam.az/site/?name=xeber&nov=maqazin');
...
foreach($html->find('.content a') as $e) { 
 ...
 $url = 'http://axsam.az/site/'.$array[1];
...
foreach($html->find('#xeber_yazi') as $e) { 
    
}

$maintext = $e->innertext.'<br><br>';

foreach($html->find('.sfa img') as $e) { 
    
  $imgs = substr($e->src, 2);
  $mainimglink[]='http://axsam.az'.$imgs;

}

  $mainimage = $mainimglink[0];

foreach($html->find('#xeber_yazi img') as $e) { 

  $imgs = substr($e->src, 2);
  $imgarray[] = "<img src='http://axsam.az".$imgs."'/><br><br>";
}

 $allimages = implode('', $imgarray);


//$video = "<iframe src='http://baku.ws".$e->src."' width='650' height='450' frameborder='0'></iframe>";
//$video = $e->outertext . '<p>';
$source='<br>'.'источник: '.'<a href="http://axsam.az" target="_blank">'.'Axsam.az'.'</a>';
$allcontent = $maintext.$allimages.$source;
$croncat = 10;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
foreach($html->find('#dle-content a') as $e) { 
  $array[] = $e->href;
}
 //echo $array[0].'<br><br>';
...
foreach($html->find('section.right_height_s h1') as $e) { 
    
   //echo  strip_tags($e->innertext,"<h1>").'<br><br>';
...
foreach($html->find('section.news_i_text') as $e) { 
   ...
   //echo  strip_tags($e->innertext,"<p>").'<br><br>';

}

$maintext = strip_tags($e->innertext,"<p>").'<br><br>';

foreach($html->find('section.news_i_text img') as $e) { 
    
   //echo  'http://az.baku.ws'.$e->src.'<br><br>';
  $imgarray[] = "<img src='http://az.baku.ws".$e->src."'/><br><br>";

  $mainimglink[]='http://az.baku.ws'.$e->src;
...
foreach($html->find('section.news_i_text iframe') as $e) { 
}
...
//$video = "<iframe src='http://baku.ws".$e->src."' width='650' height='450' frameborder='0'></iframe>";
$video = $e->outertext . '<p>';
$source='<br>'.'источник: '.'<a href="http://baku.ws" target="_blank">'.'Baku.ws'.'</a>';
$allcontent = $maintext.$allimages.$video.$source;
$croncat = 1;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
$data['lang']=0;
...
public function bakuwscron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://baku.ws/');
...
foreach($html->find('section.news_i_text img') as $e) { 
   ...
   //echo  'http://az.baku.ws'.$e->src.'<br><br>';
  $imgarray[] = "<img src='http://baku.ws".$e->src."'/><br><br>";

  $mainimglink[]='http://baku.ws'.$e->src;
...
public function milliazcron(){
    $this->load->helper('url');
    $this->load->helper('text');
    $this->load->helper('form');
    include('simple_html_dom.php');

$html = file_get_html('http://news.milli.az/');



 
foreach($html->find('.post-list2 a') as $e) { 
...
foreach($html->find('div.quiz-holder h1') as $e) { 
...
$mainimage = $e->getAttribute('src');

foreach($html->find('.content-img img') as $e) { 
    
   //echo  'http://az.baku.ws'.$e->src.'<br><br>';
  $imgarray[] = "<img width='' src='".$e->src."'/><br><br>";

 //str_replace("/250x250f","",$imgarray[]);

}

//echo $mainimage = $imgarray[0];
$allimages = implode('', str_replace("/250x250f","",$imgarray));

$source='<br>'.'источник: '.'<a href="http://milli.az" target="_blank">'.'milli.az'.'</a>';
$allcontent = $maintext.$allimages.$source;
$croncat = 1;
$data['title']=$url; // eslinde title yox urldi 
$data['img']=$mainimage;
...
*/              $data['img']=$largest_image;
                ...
                $data['basliq']= str_replace('"', '', $page_title);
...
//$data["stumble"] = $stumble;
  ...
  function newget_banners()
  {

    $this->load->view('newget_banners');
  }
	...
	{     $this->load->helper('url');
                 ...
                 if ($this->form_validation->run() == FALSE)
		{
                ...
                //$cleantitle=htmlspecialchars($this->input->post('title'), ENT_QUOTES);
                 ...
                 $cleantitle= str_replace('"', '', $this->input->post('title'));
