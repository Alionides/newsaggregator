...
<body>       
  <div class="container-fluid">
      ...
      <a href="<?php echo base_url(); ?>site/profile" class="btn btn-success">Geri qayıt</a>
      <?php echo "<p class='bg-danger'>".validation_errors('ERROR')."</p>"; ?>

      
      <br><br>
      <?php foreach ($data_ids as $val): ?>
       <!--<form class="form-vertical" role="form" action='<?php echo base_url(); ?>site/editusernews/<?php echo $val->site_id; ?>' method="post">-->
         <?php echo form_open_multipart('site/editusernews/'.$val->site_id); ?>
        <div class="form-group">
          ...
          <label for="email">Şəkil Yüklə:</label><br>
          <span class="btn btn-default btn-file">
          <input class="form-control" type="file" name="userfile">
          </span>
          <img id="uploadedimage" src="<?php echo $val->site_img_url; ?>" width="50" height="50">
          <br>

          <input type="hidden" name="hiddenimg"  value="<?php echo $val->site_img_url; ?>" />

          
          <label for="sel1">Select Category</label>
          <select name="interest" class="form-control" id="sel">
  ...
  <script>
      ...
      imageUploadURL: '<?php echo base_url();?>site/upload'})
      });
  </script>
  <script>
      $(function(){
          var catvalue = <?=$val->site_cat?>;
         document.getElementById('sel').value = catvalue;
      });

      $(function(){
          var statusvalue = <?=$val->site_status?>;
         document.getElementById('status').value = statusvalue;
      });
</script>

<script>
$('#uploadedimage').hover(function() {
    $(this).stop().animate({ width: '500px',height: '380px' }, 500);
}, function() {
    $(this).stop().animate({ width: '50px',height: '50px' }, 500);
});
